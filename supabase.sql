...
-- Una sola persona: guarda todo el negocio en una fila, protegida con PIN.
...
begin
  ...
  if exists (select 1 from reventa_state where id = 1) then
    raise exception 'YA_EXISTE';
  end if;
  insert into reventa_state (id, pin_hash, data)
  values (1, crypt(p_pin, gen_salt('bf')), '{}'::jsonb);
...
begin
  ...
  if not found then
    raise exception 'SIN_NUBE';
  end if;
  if r.pin_hash <> crypt(p_pin, r.pin_hash) then
    raise exception 'PIN_MAL';
  end if;
